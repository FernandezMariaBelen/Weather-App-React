...
const App = () => {
  ...
  const handleSubmit = (e) => {
    // si el valor input no esta vacio
    if (inputValue !== '') {
      // set ubicacion
    ...
    // si el valor input esta vacio
    if (input.value === '') {
      // animate a true
      setAnimate(true);
      // despues de 500 ms set animate a falso
      setTimeout(() => {
        setAnimate(false);
      }, 500);
    }
  ...
  useEffect(() => {
    ...
    axios
      .get(url)
      .then((res) => {
        // set the data despues de 1500 ms
        setTimeout(() => {
          setData(res.data);
  ...
  // si data es falso mostrar el loader
  if (!data) {
    return (
  ...
  // mostrar el icono de acuerdo al tiempo
  let icon;
